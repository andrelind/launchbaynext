...
import { upgradeExists } from 'lbn-core/src/helpers/unit';
import {
  FactionKey,
  Format,
  Pilot,
...
export const loadShip2 = (
  ...
  getStandardLoadout(ship.pilot);

  return ship;
};

export const getStandardLoadout = (pilot?: Pilot) => {
  if (pilot?.standardLoadout) {
    pilot.upgrades = [];
    pilot.standardLoadout.forEach((upgradeXws, index) => {
      slotKeys.forEach((slotKey) => {
        const u = upgradeData[slotKey].find(
          (upgrade) => upgrade.xws === upgradeXws
        );
        if (u) {
          pilot!.upgrades!.push(u);
        }
      });
    });
  }
