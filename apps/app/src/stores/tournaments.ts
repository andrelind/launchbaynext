...
import {
  removeTournamentOnServer,
  saveTournamentOnServer,
} from '../helpers/api';
...
export const tournamentStore = create<TournamentState>()(
  persist(
    (set, get) => ({
      tournaments: [],
      setTournaments: (tournaments: TournamentResponse[]) =>
        set({ tournaments }),
      ...
      addTournament: (tournament: TournamentResponse) => {
        ...
        saveTournamentOnServer(systemStore.getState(), tournament);
      },
      ...
      removeTournament: (id: string) => {
        const tournaments = [
          ...(get().tournaments || []).filter((t) => t.id !== id),
        ];
        set({ tournaments });

        removeTournamentOnServer(systemStore.getState(), id);
      },
  ...
  const xws: XWS = {
    ...
    pilots: x.ships.map((p) => {
      ...
      Object.keys(p.upgrades || {}).map((key) => {
        if (p.upgrades && p.upgrades?.[key as SlotKey]) {
          upgrades[key] = p.upgrades![key as SlotKey]!.map(
            (u: Upgrade) => u.xws
          );
        }
      });
    ...
    vendor: {
      lbn: {
        ...
        tags: x.tags || [],
        created: x.created
          ? parseJSON(x.created).toDateString()
