import { Source } from '../../../types';

export const t: Source[] = [
  {
    ffg: 51,
    xws: 'hugeshipconversionkit',
    name: 'Huge Ship Conversion Kit',
    wave: 6,
    released: true,
    contents: {
      ships: {},
      pilots: {
        alderaanianguard: 1,
        echobaseevacuees: 1,
        firstordercollaborators: 1,
        firstordersympathizer: 1,
        newrepublicvolunteers: 1,
        outerrimgarrison: 1,
        outerrimpatrol: 1,
        republicjudiciary: 1,
        separatistprivateers: 1,
        syndicatesmugglers: 1,
        firstordersympathizers: 1,
      },
      upgrades: {
        admiralozzel: 1,
        azmorigan: 1,
        captainneeda: 1,
        carlistrieekan: 1,
        jandodonna: 1,
        raymusantilles: 1,
        stalwartcaptain: 1,
        strategiccommander: 1,
        firecontrolsystem: 1,
        ioncannonbattery: 2,
        ordnancetubes: 2,
        pointdefensebattery: 2,
        targetingbattery: 2,
        turbolaserbattery: 2,
        heavylasercannon: 1,
        dorsalturret: 2,
        advprotontorpedoes: 2,
        clustermissiles: 2,
        concussionmissiles: 2,
        novicetechnician: 1,
        seasonednavigator: 1,
        torynfarr: 1,
        agilegunner: 2,
        hotshotgunner: 2,
        adaptiveshields: 2,
        boostedscanners: 2,
        optimizedpowercore: 2,
        tibannareserves: 2,
        bombardmentspecialists: 2,
        commsteam: 2,
        damagecontrolteam: 2,
        gunneryspecialists: 2,
        igrmdroids: 2,
        ordnanceteam: 2,
        sensorexperts: 2,
        quickreleaselocks: 1,
        saboteursmap: 1,
        scannerbaffler: 1,
        proximitymines: 1,
        assailer: 1,
        bloodcrow: 1,
        brighthope: 1,
        brokenhorn: 1,
        corvus: 1,
        dodonnaspride: 1,
        impetuous: 1,
        insatiableworrt: 1,
        instigator: 1,
        jainaslight: 1,
        liberator: 1,
        luminous: 1,
        merchantone: 1,
        quantumstorm: 1,
        requiem: 1,
        suppressor: 1,
        tantiveiv: 1,
        thunderstrike: 1,
        vector: 1,
        corsairrefit: 2,
        seismiccharges: 1,
      },
    },
  },
  {
    ffg: 54,
    xws: 'tantiveivexpansionpack',
    name: 'Tantive IV Expansion Pack',
    wave: 6,
    released: true,
    contents: {
      ships: { cr90corelliancorvette: 1 },
      pilots: { alderaanianguard: 1, republicjudiciary: 1 },
      upgrades: {
        carlistrieekan: 1,
        jandodonna: 1,
        raymusantilles: 1,
        stalwartcaptain: 1,
        strategiccommander: 1,
        ioncannonbattery: 1,
        pointdefensebattery: 1,
        targetingbattery: 1,
        turbolaserbattery: 1,
        novicetechnician: 1,
        seasonednavigator: 1,
        torynfarr: 1,
        agilegunner: 1,
        hotshotgunner: 1,
        adaptiveshields: 1,
        boostedscanners: 1,
        optimizedpowercore: 1,
        tibannareserves: 1,
        bombardmentspecialists: 1,
        commsteam: 1,
        damagecontrolteam: 1,
        gunneryspecialists: 1,
        sensorexperts: 1,
        dodonnaspride: 1,
        jainaslight: 1,
        liberator: 1,
        tantiveiv: 1,
        thunderstrike: 1,
      },
    },
  },
  {
    ffg: 55,
    xws: 'croccruiserexpansionpack',
    name: 'C-ROC Cruiser Expansion Pack',
    wave: 6,
    released: true,
    contents: {
      ships: { croccruiser: 1 },
      pilots: { separatistprivateers: 1, syndicatesmugglers: 1 },
      upgrades: {
        azimorigan: 1,
        stalwartcaptain: 1,
        strategiccommander: 1,
        firecontrolsystem: 1,
        ioncannonbattery: 1,
        ordnancetubes: 1,
        pointdefensebattery: 1,
        targetingbattery: 1,
        turbolaserbattery: 1,
        heavylasercannon: 1,
        dorsalturret: 1,
        clustermissiles: 1,
        concussionmissiles: 1,
        novicetechnician: 1,
        seasonednavigator: 1,
        agilegunner: 1,
        hotshotgunner: 1,
        adaptiveshields: 1,
        boostedscanners: 1,
        optimizedpowercore: 1,
        tibannareserves: 1,
        bombardmentspecialists: 1,
        commsteam: 1,
        damagecontrolteam: 1,
        gunneryspecialists: 1,
        igrmdroids: 1,
        ordnanceteam: 1,
        sensorexperts: 1,
        quickreleaselocks: 1,
        saboteursmap: 1,
        scannerbaffler: 1,
        proximitymines: 1,
        brokenhorn: 1,
        insatiableworrt: 1,
        merchantone: 1,
        corsairrefit: 1,
        seismiccharges: 1,
        azmorigan: 1,
      },
    },
  },
  {
    ffg: 53,
    xws: 'imperialraiderexpansionpack',
    name: 'Imperial Raider Expansion Pack',
    wave: 6,
    released: true,
    contents: {
      ships: { raiderclasscorvette: 1 },
      pilots: { firstordercollaborators: 1, outerrimpatrol: 1 },
      upgrades: {
        admiralozzel: 1,
        captainneeda: 1,
        stalwartcaptain: 1,
        strategiccommander: 1,
        ioncannonbattery: 1,
        ordnancetubes: 1,
        pointdefensebattery: 1,
        targetingbattery: 1,
        turbolaserbattery: 1,
        advprotontorpedoes: 1,
        clustermissiles: 1,
        concussionmissiles: 1,
        novicetechnician: 1,
        seasonednavigator: 1,
        agilegunner: 1,
        hotshotgunner: 1,
        adaptiveshields: 1,
        boostedscanners: 1,
        optimizedpowercore: 1,
        tibannareserves: 1,
        bombardmentspecialists: 1,
        commsteam: 1,
        damagecontrolteam: 1,
        gunneryspecialists: 1,
        ordnanceteam: 1,
        sensorexperts: 1,
        quickreleaselocks: 1,
        saboteursmap: 1,
        scannerbaffler: 1,
        proximitymines: 1,
        assailer: 1,
        corvus: 1,
        impetuous: 1,
        instigator: 1,
      },
    },
  },
  {
    ffg: 52,
    xws: 'epicbattles',
    name: 'Epic Battles Expansion',
    wave: 6,
    released: true,
    contents: {
      ships: {},
      pilots: {},
      upgrades: {
        agentoftheempire: 1,
        firstorderelite: 1,
        veteranwingleader: 4,
        dreadnoughthunter: 2,
      },
    },
  },
  {
    ffg: 68,
    xws: 'tridentclassassaultship',
    name: 'Trident-class Assault Ship',
    wave: 9,
    released: true,
    contents: {
      ships: { tridentclassassaultship: 1 },
      pilots: { colicoiddestroyer: 1, lawlesspirates: 1 },
      upgrades: {
        novicetechnician: 1,
        seasonednavigator: 1,
        hotshotgunner: 1,
        asajjventress: 1,
        'generalgrievous-command': 1,
        martuuk: 1,
        rifftamson: 1,
        zealouscaptain: 1,
        trackingtorpedoes: 1,
        droidcrew: 1,
        agilegunner: 1,
        corsaircrew: 1,
        grappler: 1,
        strategiccommander: 1,
        ioncannonbattery: 1,
        targetingbattery: 1,
        ordnancetubes: 1,
        pointdefensebattery: 1,
        bombardmentspecialists: 1,
        commsteam: 1,
        damagecontrolteam: 1,
        trident: 1,
        stalwartcaptain: 1,
        neimoidiangrasp: 1,
        'hondoohnaka-command': 1,
        tractortechnicians: 1,
        nautolansrevenge: 1,
        tractortentacles: 1,
        drillbeak: 1,
        protoncannonbattery: 1,
        enhancedpropulsion: 1,
      },
    },
  },
];

export default t;
