import { ShipType } from '../../../../types';

const t: ShipType = {
  name: 'VCX-100 Light Freighter',
  xws: 'vcx100lightfreighter',
  ffg: 23,
  size: 'Large',
  dial: [
    '1TR',
    '1BW',
    '1FB',
    '1NW',
    '1YR',
    '2TW',
    '2BB',
    '2FB',
    '2NB',
    '2YW',
    '3TR',
    '3BW',
    '3FW',
    '3NW',
    '3YR',
    '4FW',
    '4KR',
  ],
  faction: 'Rebel Alliance',
  stats: [
    { arc: 'Front Arc', type: 'attack', value: 4 },
    { type: 'agility', value: 0 },
    { type: 'hull', value: 10 },
    { type: 'shields', value: 4 },
  ],
  actions: [
    { difficulty: 'White', type: 'Focus' },
    { difficulty: 'White', type: 'Lock' },
    { difficulty: 'White', type: 'Reinforce' },
  ],
  ability: {
    name: 'Tail Gun',
    text: "While you have a docked ship, you have a primary [Rear Arc] weapon with an attack value equal to your docked ship's primary [Front Arc] attack value.",
  },
  icon: 'https://infinitearenas.com/xw2/images/shipicons/rebel-alliance/I_Ghost.png',
  pilots: [
    {
      name: '“Chopper”',
      caption: 'Spectre-3',
      initiative: 2,
      limited: 1,
      cost: 16,
      xws: 'chopper',
      ability:
        'At the start of the Engagement Phase, each enemy ship at range 0 gains 2 jam tokens.',
      shipActions: [
        { difficulty: 'White', type: 'Calculate' },
        { difficulty: 'White', type: 'Lock' },
        { difficulty: 'White', type: 'Reinforce' },
      ],
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
        'Title',
      ],
      ffg: 75,
      standard: true,
      epic: true,
      loadout: 12,
      extended: true,
      keywords: ['Droid', 'Freighter', 'Spectre'],
      image: 'https://infinitearenas.com/xw2/images/pilots/chopper.png',
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/chopper.png',
    },
    {
      name: 'Hera Syndulla',
      caption: 'Spectre-2',
      initiative: 5,
      limited: 1,
      cost: 17,
      loadout: 19,
      xws: 'herasyndulla-vcx100lightfreighter',
      ability:
        'After you reveal a red or blue maneuver, you may set your dial to another maneuver of the same difficulty.',
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
        'Title',
        'Talent',
        'Talent',
      ],
      ffg: 73,
      standard: false,
      epic: true,
      extended: true,
      keywords: ['Freighter', 'Spectre'],
      image:
        'https://infinitearenas.com/xw2/images/pilots/herasyndulla-vcx100lightfreighter.png',
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/herasyndulla-vcx100lightfreighter.png',
    },
    {
      name: 'Hera Syndulla',
      caption: 'Legends and Relics',
      initiative: 6,
      limited: 1,
      cost: 18,
      loadout: 20,
      xws: 'herasyndulla-legendsandrelics',
      ability:
        'After you fully execute a red maneuver, you may coordinate a friendly ship at range 0-3.',
      image:
        'https://infinitearenas.com/xw2xwa/images/pilots/herasyndulla-legendsandrelics.png',
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
        'Title',
        'Talent',
        'Talent',
      ],
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/herasyndulla-legendsandrelics.png',
      standard: true,
      extended: true,
      keywords: ['Freighter', 'Spectre'],
      epic: true,
      id: 732,
      ffg: 1161,
    },
    {
      name: 'Kanan Jarrus',
      caption: 'Spectre-1',
      initiative: 3,
      limited: 1,
      cost: 18,
      xws: 'kananjarrus',
      ability:
        'While a friendly ship in your firing arc defends, you may spend 1 [Force]. If you do, the attacker rolls 1 fewer attack die.',
      force: { value: 2, recovers: 1, side: ['light'] },
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
        'Title',
        'Force Power',
      ],
      ffg: 74,
      standard: true,
      epic: true,
      loadout: 20,
      extended: true,
      keywords: ['Freighter', 'Jedi', 'Light Side', 'Spectre'],
      image: 'https://infinitearenas.com/xw2/images/pilots/kananjarrus.png',
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/kananjarrus.png',
    },
    {
      name: 'Lothal Rebel',
      initiative: 2,
      limited: 0,
      cost: 15,
      xws: 'lothalrebel',
      text: 'Another successful Corellian Engineering Corporation freighter design, the VCX-100 is larger than the ubiquitous YT-series, boasting more living space and customizability.',
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
      ],
      ffg: 76,
      standard: true,
      epic: true,
      keywords: ['Freighter'],
      loadout: 9,
      extended: true,
      image: 'https://infinitearenas.com/xw2/images/pilots/lothalrebel.png',
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/lothalrebel.png',
      caption: '',
    },
    {
      name: 'Alexsandr Kallus',
      xws: 'alexsandrkallus',
      initiative: 4,
      limited: 1,
      caption: 'Fulcrum',
      standard: true,
      epic: true,
      slots: [
        'Crew',
        'Crew',
        'Sensor',
        'Gunner',
        'Turret',
        'Modification',
        'Torpedo',
        'Title',
        'Talent',
      ],
      ability:
        'While you defend, if the attacker modified any attack dice, you may roll 1 additional defense die.',
      cost: 17,
      ffg: 631,
      loadout: 14,
      extended: true,
      keywords: ['Freighter', 'Spectre'],
      image: 'https://infinitearenas.com/xw2/images/pilots/alexsandrkallus.png',
      artwork:
        'https://infinitearenas.com/xw2/images/artwork/pilots/alexsandrkallus.png',
    },
  ],
};

export default t;
