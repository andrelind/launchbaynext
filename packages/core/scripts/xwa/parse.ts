...
const runner = async () => {
  for await (const faction of [
    ...
    Object.keys(ships).forEach(shipName => {
      Object.keys(pilotData[factionFromKey(faction as FactionKey)]).forEach(async shipKey => {
        ...
        const formatted = await prettier.format(`${header}${JSON.stringify(shipData)};\n\nexport default t;`, {
          trailingComma: 'all',
          singleQuote: true,
          parser: 'typescript',
        });
  ...
  const upgrades = JSON.parse(upgradeFile) as {
    [key: string]: {
      name: string;
      cost: number;
      limited: number;
        ...
        up.restrictions = upgrade.restrictions
          ...
          .map(restriction => {
            if (restriction.factions) {
              return { factions: restriction.factions };
              // @ts-expect-error
  ...
  for await (const slotKey of Object.keys(upgradeData)) {
    ...
    const formatted = await prettier.format(`${header}${JSON.stringify(upgrade)};\n\nexport default t;`, {
      trailingComma: 'all',
      singleQuote: true,
      parser: 'typescript',
    });

    await fs.writeFile(`../../src/assets/xwa/upgrades/${slotKey}.ts`, formatted, 'utf8');
  }
};
